...
test.describe('Basic functionality', () => {
  test('toast is rendered from the hero button and disappears after the default timeout', async ({ page }) => {
  ...
  test('loading toast updates in place to success', async ({ page }) => {
    ...
    expect(await initialHandle?.evaluate((node, candidate) => node === candidate, updatedHandle)).toBe(true)
  })
  ...
  test('toast is removed after swiping down', async ({ page }) => {
    ...
    if (!box)
      throw new Error('Toast bounding box missing')
  ...
  test('position controls move the active toaster', async ({ page }) => {
    ...
    await expect(page.locator('[data-sonner-toaster]').first()).toHaveAttribute('data-x-position', 'left')
    await expect(page.locator('[data-sonner-toaster]').first()).toHaveAttribute('data-y-position', 'top')
  })
  ...
  test('rich colors can be enabled from the docs controls', async ({ page }) => {
    ...
    await expect(page.locator('[data-sonner-toast]').first()).toHaveAttribute('data-rich-colors', 'true')
  })
