  ...
  ;[defaultOffset, mobileOffset].forEach((offset, index) => {
    ...
    if (typeof offset === 'object' && offset !== null) {
      for (const key of ['top', 'right', 'bottom', 'left'] as const) {
        ...
        styles[`${prefix}-${key}`] = value === undefined ? defaultValue : typeof value === 'number' ? `${value}px` : value
      }
      return
    }
...
function Toast(props: ToastProps) {
  ...
  function getLoadingIcon() {
    if (props.icons?.loading) {
      return (
        <div class={cn(classNames()?.loader, toastClassNames()?.loader, 'sonner-loader')} data-visible={toastType() === 'loading'}>
          {props.icons.loading}
        </div>
      )
    }
  ...
  const icon = () => props.toast.icon ?? props.icons?.[toastType() as keyof ToastIcons] ?? getAsset(toastType())
  ...
  createEffect(
    on(
      () => [props.expanded, props.interacting, props.toast, toastType(), isDocumentHidden(), duration()] as const,
  ...
  return (
    ...
    >
      ...
      <Show when={(toastType() || props.toast.icon || props.toast.promise) && props.toast.icon !== null && (props.icons?.[toastType() as keyof ToastIcons] !== null || props.toast.icon)}>
      ...
      <Show when={props.toast.cancel}>
        {cancel => (
          <Show when={!isAction(cancel())} fallback={(
            <button
              data-button
              data-cancel
      ...
      <Show when={props.toast.action}>
        {action => (
          <Show when={!isAction(action())} fallback={(
            <button
              data-button
              data-action
...
function Toaster(props: ToasterProps) {
  ...
  const possiblePositions = createMemo(() => {
    return Array.from(new Set([propsWithDefaults.position, ...filteredToasts().filter(toast => toast.position).map(toast => toast.position as Position)]))
  })
  ...
  onMount(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      const isHotkeyPressed = propsWithDefaults.hotkey.length > 0 && propsWithDefaults.hotkey.every(key => (event as any)[key] || event.code === key)
          ...
          return (
            <Show when={filteredToasts().length > 0}>
              <ol
                ...
                onBlur={(event) => {
                  if (isFocusWithinRef() && !event.currentTarget.contains(event.relatedTarget as Node | null)) {
                    setIsFocusWithinRef(false)
                    if (lastFocusedElementRef()) {
                      lastFocusedElementRef()?.focus({ preventScroll: true })
                      setLastFocusedElementRef(null)
